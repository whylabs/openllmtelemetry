...
class OpenAIV1Instrumentor(BaseInstrumentor):
    ...
    def _instrument(self, **kwargs):
        ...
        wrap_function_wrapper(
            ...
            "Completions.create",
            chat_wrapper(tracer, self._guard),
        )
        wrap_function_wrapper(
            "openai.resources.completions",
            "Completions.create",
            completion_wrapper(tracer, self._guard),
        )
        wrap_function_wrapper(
            "openai.resources.embeddings",
            "Embeddings.create",
            embeddings_wrapper(tracer, self._guard),
        )
        wrap_function_wrapper(
            "openai.resources.chat.completions",
            "AsyncCompletions.create",
            achat_wrapper(tracer, self._guard),
        )
        wrap_function_wrapper(
            "openai.resources.completions",
            "AsyncCompletions.create",
            acompletion_wrapper(tracer, self._guard),
        )
        wrap_function_wrapper(
            "openai.resources.embeddings",
            "AsyncEmbeddings.create",
            aembeddings_wrapper(tracer, self._guard),
        )
